...
import {
  useTheme, Layout, List, Card, Text, Button, OverflowMenu,
  MenuItem, Icon,
      ...
      headerComponent={
        <Layout style={{backgroundColor: theme['color-primary-500']}}>
          ...
          <View style={[cStyles.itemsCenter, cStyles.px16, cStyles.pb16]}>
            <Text status={'control'} category={'h1'}>{`${dataQuiz.score}`}</Text>
            <Text style={cStyles.mt6} status={'control'} category={'p1'}>{t('quiz_review:total_score')}</Text>
          </View>
        </Layout>
      }>
      <List
        ...
        renderItem={info => {
          return (
            <Card
              status={info.item.isCorrect ? 'success' : 'danger'}
              disabled
              header={(propsH) => (
                <View style={[cStyles.flex1, cStyles.row, cStyles.itemsStart, cStyles.justifyBetween, cStyles.p10]}>
                  <View style={[cStyles.itemsCenter, {flex: 0.05}]}><Text category={'label'}>{info.index + 1}.</Text></View>
                    ...
                    {typeof info.item.question === 'object'
                      ? (info.item.question.map((itemQus, indexQus) => {
                        if (indexQus === info.item.question.length - 1) {
                          return <Text category={'label'}>{itemQus}</Text>
                        }
                        if (indexQus >= 0) {
                          return (
                            <View style={[cStyles.row, cStyles.itemsCenter]}>
                              <Text category={'label'}>{itemQus}</Text>
                      ...
                      :
                      <Text category={'label'}>{info.item.question}</Text>
                    }
                    
                    <Text style={cStyles.mt10} category={'c1'}>{t('quiz_review:correct_answer')}:</Text>
                    {info.item.type === TYPE_QUESTION.TEXT_INPUT && (
                      <Text style={cStyles.mt10} category={'p1'}>{info.item.correctAnswer}</Text>
                    )}
                    {info.item.type === TYPE_QUESTION.YES_NO && (
                      <Text style={cStyles.mt10} category={'p1'}>{t('quiz_review:' + info.item.correctAnswer)}</Text>
                    )}
                    {info.item.type === TYPE_QUESTION.CHOOSE_ONE && (
                      <Text style={cStyles.mt10} category={'p1'}>&#10003; {info.item.answers[info.item.correctAnswer]}</Text>
                    )}
                    {info.item.type === TYPE_QUESTION.CHOOSE_MULTI && info.item.correctAnswer.map((itemAns, indexAns) => (
                      <Text key={itemAns + '_' + indexAns} style={cStyles.mt10} category={'p1'}>
                    ...
                    {info.item.type === TYPE_QUESTION.SELECT_BLANK && (
                      <View style={[cStyles.row, cStyles.itemsCenter]}>
                        {info.item.correctAnswer.map((itemAns, indexAns) => (
                          <Text key={itemAns + '_' + indexAns} style={[cStyles.mt10, cStyles.ml10]} category={'p1'}>
                            &#10003; {info.item.answers[itemAns]}
                          </Text>
                        ))}
                      </View>
                      ...
                      <View style={[cStyles.row, cStyles.itemsCenter, cStyles.flexWrap]}>
                        {info.item.correctAnswer.map((itemAns, indexAns) => (
                          <Layout style={[cStyles.center, cStyles.px10, cStyles.py4, cStyles.mt5, cStyles.mr5, cStyles.rounded1]} level={'4'}>
                            <Text key={itemAns + '_' + indexAns} category={'p1'}>
                              {info.item.answers[itemAns]}
                            </Text>
                          </Layout>
                        ))}
                  ...
                  <View style={cStyles.itemsCenter}>
                    <View style={[cStyles.row, cStyles.itemsCenter]}>
                      <IoniIcon
                        ...
                        size={moderateScale(16)}
                      />
                      <Text style={cStyles.ml5} category={'label'} status={info.item.isCorrect ? 'success' : 'danger'}>
                        {t('quiz_review:' + (info.item.isCorrect ? 'true' : 'false')).toUpperCase()}
                      </Text>
                    </View>
                  </View>
                  <View style={[cStyles.row, cStyles.itemsCenter]}>
                    <Text style={cStyles.mr5} category={'c1'}>{t('quiz_review:score')}</Text>
                    <Button size={'small'} appearance={'filled'} status={'basic'}>{info.item.score}</Button>
                  </View>
                </View>
              )}
            >
              <Text category={'c1'}>{t('quiz_review:your_answer')}:</Text>
              {info.item.type === TYPE_QUESTION.TEXT_INPUT && (
                <Text style={cStyles.mt10} category={'p1'}>{info.item.userAnswer}</Text>
              )}
              {info.item.type === TYPE_QUESTION.YES_NO && (
                <Text style={cStyles.mt10} category={'p1'}>{t('quiz_review:' + info.item.userAnswer)}</Text>
              )}
              {info.item.type === TYPE_QUESTION.CHOOSE_ONE && (
                <Text style={cStyles.mt10} category={'p1'}>{info.item.answers[info.item.userAnswer]}</Text>
              )}
