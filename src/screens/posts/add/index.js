...
import {
  Button, CheckBox, Divider, Input, Layout, Text, TopNavigationAction,
  Menu, MenuItem, Icon, useTheme,
} from '@ui-kitten/components';
import {Alert, ScrollView, StyleSheet, View} from 'react-native';
import DocumentPicker from 'react-native-document-picker';
import {launchCamera, launchImageLibrary} from 'react-native-image-picker';
import {checkMultiple, request, openSettings, PERMISSIONS, RESULTS} from 'react-native-permissions';
...
/* COMMON */
import {IS_ANDROID, IS_IOS, moderateScale, SCREEN_HEIGHT, SCREEN_WIDTH, sW} from '~/utils/helper';
...
const RenderPublishText = (props) => {
  ...
  return (
    <Text status={'primary'} category={'s1'}>{t('add_post:publish')}</Text>
  );
};
    ...
    <CContainer
      ...
      <ScrollView style={cStyles.flex1}>
        <Layout style={cStyles.p16} level={'1'}>
          ...
          <Text category={'label'}>{t('add_post:attached')}</Text>
                ...
                >
                  {evaProps => (
                    <View style={cStyles.itemsCenter}>
                      ...
                      />
                      <Text style={cStyles.mt10} category={'p1'}>{t('add_post:add_file')}</Text>
                    </View>
                  )}
                </Button>
                <Button
                  ...
                  {evaProps => (
                    <View style={cStyles.itemsCenter}>
                      <IoniIcon
                        ...
                        color={colors.PRIMARY}
                      />
                      <Text style={cStyles.mt10} category={'p1'}>{t('add_post:add_image')}</Text>
                    </View>
                  )}
                </Button>
              </View>
                  ...
                  <Layout style={[cStyles.flex1, cStyles.row, cStyles.itemsCenter, cStyles.justifyBetween, cStyles.mt6, cStyles.pl16, cStyles.pr10, cStyles.py10, cStyles.rounded1]} level={'2'}>
                    <View style={[cStyles.flex1, cStyles.row, cStyles.itemsCenter]}>
                      ...
                      <View style={[cStyles.flex1, cStyles.ml8]}>
                        <Text category={'p1'} numberOfLines={2}>{itemAsset.name}</Text>
                        <Text style={cStyles.mt5} category={'c1'} numberOfLines={1}>{itemAsset.size}</Text>
                      </View>
                    </View>
          ...
          <Text category={'label'}>{t('add_post:level')}</Text>
        ...
        <Layout level={'1'}>
          <SafeAreaView>
            <View style={cStyles.m16}>
              <Text category={'p1'}>{t('common:title_choose_photos_videos')}</Text>
            </View>
            <Menu>
              <MenuItem
                title='Camera'
